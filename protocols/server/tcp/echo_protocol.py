    ...
    def on_message(self, address, message):
        ...
        if message:
            ...
            if 'add_device' in message:
                ...
                self.server.d_b_connector.on_device_message(dev_id, {'dev_id': dev_id, 'connected': 1, 'address': address})
                ...
                self.server.d_b_connector.on_device_message(dev_id, {'dev_id': dev_id, 'connected': 1, 'address': message['new_address']})
                self.send(address, message)
            else:
                self.send(address, message)
